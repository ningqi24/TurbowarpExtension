...
// Yes shape中文翻译 <https://space.bilibili.com/1277463188>
// 许可证: MIT
...
(function(Scratch) {
  "use strict";
  if (!Scratch.extensions.unsandboxed) throw new Error("高级自定义块必须在非沙盒模式下运行！");
  ...
  const TEMP_BLOCK_OPCODE = "SPmbpCST_$-,.___SPmbpCST_TEMP_BLOCK"; // 添加到项目中以保持扩展存在的临时块的opcode
  const CUSTOM_MENU_ID = "SP0zMenuMaker_menu_";
  const targetProcData = Symbol("MBPprocedureData"); // 存储重要的全局过程数据的位置

  let proceduresXML = "", tempStore = {}, storage = {};
  let globalBlocksCache = {};
  let globalTargetFocus = undefined; // 不要与globalBlocksCache混淆，这有助于在堆栈中查找块ID

  let imgStorage = {}, imgStoreSize = 0;

  let extensionRemovable = false;
  let ext; // 扩展对象
  let execute, Thread; // 由exports设置
  ...
  const inputTypes = {
    ...
    // %n技术上已经存在，但scratch-blocks的某些部分会将其替换为%s，所以我们不能使用它
  ...
  // 如果为true，滚动到"我的块"将改为滚动到"高级自定义块"
  // 用于使"块"对话框滚动到MB+类别
  let shouldScrollToMBP = false;

  // 当调用时为true时刷新块列表，否则不执行任何操作
  ...
  function openBlockMaker(workspace, isEditing) {
    ...
    // Scratch为自定义块新工作区，我们需要捕获它
    const newWorkspace = ScratchBlocks.mainWorkspace;
    const modal = document.querySelector(`div[class="ReactModalPortal"]`);
    modal.querySelector(`div[class*="modal_header-item-title"]`).textContent = "制作新的高级积木";
  ...
  function genDrpButton(container, opts, defaultValue, isDark, isLabel, changeFunc, setFunc) {
    ...
    // 在Firefox上，"click"事件会传播到按钮，从而添加另一个输入。不要这样做
  ...
  function attachCheckboxes(modal, editor, isEditing, ogProcode) {
    ...
    checkRow.append(isPM && !isEditing ? "" : document.createElement("br"), makeBox(
      "栈块", tempStore.isTerminal !== undefined ? tempStore.isTerminal : false,
      (val) => {
    ...
    checkRow.append(document.createElement("br"), makeBox(
      "全局积木", tempStore.global !== undefined ? tempStore.global : false,
  ...
  function openImgSelector(editor, isDark, ogWorkspace) {
    ...
    openModal(
      ...
      (modal, modalStorage) => {
        ...
        text.textContent = "自定义图片";
        dropItem.append(img, text);

        dropDiv.appendChild(dropItem);
        dropItem.addEventListener("click", () => {
          const src = prompt("输入您的图片URL或Data.URI:");
          if (src) {
      ...
      (value) => {
        ...
        const args = editor.argumentIds_;
        const id = args[args.length - 1];
        editor.displayNames_[args.length - 1] = "图片";
        editor.updateDisplay_();
        editor.focusLastEditor_();
  ...
  if (exports) {
    const sanitize = string => {
      if (typeof string !== "string") {
        console.warn(`sanitize收到意外类型: ${typeof string}`);
        string = "" + string;
      }
  ...
  if (Scratch.gui) Scratch.gui.getBlockly().then(SB => {
    ...
    utils.updateDisplay_ = function() {
      ...
      // 在PM中更改输出类型时，断开并重新连接原型与定义块
      // 否则会发生错误，我们必须一个新的定义块来更改其类型
      let defineBlock = (
      ...
      if (store?.inputs && this.type !== "procedures_declaration") for (let i = this.inputList.length; i--;) {
        ...
        if (
          ...
          if (input.connection && input.connection.isConnected()) {
            ...
            if (block.isShadow()) block.dispose(); // 销毁任何附加的影子块
            else block.unplug(); // 断开任何附加的正常块
          }
          input.dispose();
          // ...但在原位置用新输入替换它
          //（调用removeInput()会移位inputList的所有元素，这有点慢）
      ...
      if (!this.isInsertionMarker()) {
        // ScratchBlocks 获取下一个块的方法在块时不起作用
    ...
    // 将标记为顶部块的过程调用转换为顶部块（如果它们可以转换为顶部块）
  ...
  function startBlockListener() {
    runtime.on("BLOCK_DRAG_END", (newStack, oldID) => {
      ...
      if (copyStore) setTimeout(() => {
        // 对于这种情况，setTimeout比queueMicrotask更好？
        for (const target of runtime.targets) {
  ...
  function compileProcedures(xmlList, workspace, utils) {
    ...
    if (globalProcs.size > 0) {
      let tempList = `<sep gap="12"/><label text="全局块"/><sep gap="6"/>`;
      ...
      tempList += `<sep gap="6"/><label text="私有块"/><sep gap="6"/>`;
  ...
  vm._addSprite3 = function(...args) {
    ...
    // 如果存在则移除虚拟块，这从来不是舞台（保存的地方），所以不会发生什么坏事
  ...
  class SPmbpCST {
      ...
      return {
        ...
        blocks: [
          ...
          },
          {
            ...
            hideFromPalette: extensionRemovable,
            arguments: {
              PARAM: { type: Scratch.ArgumentType.STRING, defaultValue: "参数名" }
            },
          },
          {
            opcode: "runBranch",
    ...
    removeExt() {
      if (confirm("移除此扩展？这将移除自制积木的所有新增的功能"))
        removeExtension();
    }
    ...
    getProcedureParamMenu() {
      ...
      if (!innerBlock || !innerBlock.type == "procedures_prototype") return ["（无效的定义块！）"];
    ...
    evalParam(args, util) {
      if (!Thread || !execute) {
        console.error("高级自定义块无法访问Exports！");
        return;
      }
